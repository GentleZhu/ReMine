...
class PreProcessor(object):
    ...
    def chunk_train(self, docIn, posIn):
        ...
        cnt = 0
        with open(docIn, encoding='utf-8') as doc, open(posIn, encoding='utf-8') as pos, open(
                'tmp_remine/boost_patterns.txt', 'w', encoding='utf-8') as out:
                    ...
                    if subtree.label() == 'NP' or subtree.label() == 'DATE':
                        ...
                        if len(branch) > 1:
                            ...
                            if 'www' not in string and '.com' not in string:
                                out.write(' '.join(
                                    list(map(lambda x: str(self.inWordmapping(x[0])) + '_' + x[1], branch))) + '\n')
    ...
    def tokenized_test(self, docIn, posIn, depIn):
        ...
        self.test_tokens = []
        self.test_words = []
        with open(docIn, encoding='utf-8') as doc, open(posIn, encoding='utf-8') as pos, open(depIn,
                                                                                              encoding='utf-8') as dep:
    ...
    def load(self):
        self.word_mapping = _pickle.load(open('tmp_remine/token_mapping.p', 'rb',protocol = 2))
