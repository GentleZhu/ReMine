...
// ===
    ...
    inline void mine_pos(int MIN_SUP, int LENGTH_THRESHOLD = 6) {
        ...
        */
        ...
        id2ends_tag.clear();
        patterns_tag.clear();
        pattern2id_tag.clear();
        //id2ends.shrink_to_fit();
        //patterns.shrink_to_fit();
        //pattern2id.shrink_to_fit();

        //delete[] unigrams;
        unigrams_tag = new TOTAL_TOKENS_TYPE[Documents::maxPosID + 1];
        
        # pragma omp parallel for schedule(dynamic, PATTERN_CHUNK_SIZE)
        for (TOTAL_TOKENS_TYPE i = 0; i <= Documents::maxPosID; ++ i) {
            unigrams_tag[i] = 0;
        }
        # pragma omp parallel for schedule(dynamic, PATTERN_CHUNK_SIZE)
        for (TOTAL_TOKENS_TYPE i = 0; i < Documents::totalWordTokens; ++ i) {
        ...
        // allocate memory
        for (TOTAL_TOKENS_TYPE i = 0; i <= Documents::maxPosID; ++ i) {
            pattern2id_tag[i + 1] = patterns_tag.size();
            patterns_tag.push_back(Pattern());
        }
        id2ends_tag.resize(patterns.size());
        # pragma omp parallel for schedule(dynamic, PATTERN_CHUNK_SIZE)
        for (TOTAL_TOKENS_TYPE i = 0; i <= Documents::maxPosID; ++ i) {
            id2ends_tag[i].resize(unigrams_tag[i] >= MIN_SUP ? unigrams_tag[i] : 0);
        }
        ...
        for (TOTAL_TOKENS_TYPE i = 0; i < Documents::totalWordTokens; ++ i) {
            if (!pruneByPOSTag(i, i)) {
                const TOTAL_TOKENS_TYPE& token = Documents::posTags[i];
                addPatternPos(Pattern(token), i, unigrams_tag[token] >= MIN_SUP);
                totalOcc += unigrams_tag[token] >= MIN_SUP;
            }
        }
        ...
        PATTERN_ID_TYPE last = 0;
        for (int len = 1; len <= LENGTH_THRESHOLD && last < patterns_tag.size(); ++ len) {
            ...
            # pragma omp parallel for schedule(dynamic, PATTERN_CHUNK_SIZE)
            for (PATTERN_ID_TYPE id = last; id < backup; ++ id) {
                assert(patterns_tag[id].size() == 0 || patterns_tag[id].size() == len);
                id2ends_tag[id].shrink_to_fit();
                if (len < LENGTH_THRESHOLD) {
                    for (const TOTAL_TOKENS_TYPE& ed : id2ends_tag[id]) {
                        TOTAL_TOKENS_TYPE st = ed - len + 1;
            ...
            for (int tid = 0; tid < NTHREADS; ++ tid) {
                for (const auto& iter : threadFreq[tid]) {
                    ...
                    if (freq >= MIN_SUP) {
                        const ULL& hashValue = iter.first;
                        pattern2id_tag[hashValue] = patterns_tag.size();
                        patterns_tag.push_back(Pattern());
                        newPatterns.push_back(make_pair(hashValue, freq));
                    }
                }
            ...
            id2ends_tag.resize(patterns_tag.size());
            ...
            for (size_t i = 0; i < newPatterns.size(); ++ i) {
                const ULL& hashValue = newPatterns[i].first;
                const TOTAL_TOKENS_TYPE& freq = newPatterns[i].second;
                id2ends_tag[pattern2id_tag[hashValue]].resize(freq);
            }
            newPatterns.clear();
            ...
            for (PATTERN_ID_TYPE id = last; id < backup; ++ id) {
                if (len < LENGTH_THRESHOLD) {
                    vector<TOTAL_TOKENS_TYPE> positions = id2ends_tag[id];
                    for (const TOTAL_TOKENS_TYPE& ed : positions) {
                        TOTAL_TOKENS_TYPE st = ed - len + 1;
                        assert(Documents::posTags[st] == patterns_tag[id].tokens[0]);
                        ...
                        //if (!Documents::isEndOfSentence(ed) || Documents::punctuations[Documents::wordTokens[ed + 1]] == "," ) {
                            if (!pruneByPOSTag(st, ed + 1) && unigrams_tag[Documents::posTags[ed + 1]] >= MIN_SUP) {
                                ...
                                if (pattern2id_tag.count(newHashValue)) {
                                    Pattern newPattern(patterns_tag[id]);
                                    newPattern.append(Documents::posTags[ed + 1]);
                                    assert(newPattern.size() == len + 1);
                                    newPattern.currentFreq = 0;

                                    addPatternWithoutLocksPos(newPattern, ed + 1);
                                    ++ totalOcc;
                                }
                            }
                        }
        ...
        id2ends_tag.shrink_to_fit();

        cerr << "# of frequent patterns = " << patterns_tag.size() << endl;
        cerr << "total occurrence = " << totalOcc << endl;

        for (int i = 0; i < patterns_tag.size(); ++ i) {
            assert(patterns_tag[i].currentFreq == id2ends_tag[i].size() || id2ends_tag[i].size() == 0);
            assert(patterns_tag[i].size() == 0 || patterns_tag[i].size() == 1 || id2ends_tag[i].size() >= MIN_SUP);
        }
    }
};
