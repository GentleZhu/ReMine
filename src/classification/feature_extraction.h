...
// ===
    ...
    void extractStatistical_tag(int id, vector<double> &feature) {
        const Pattern &pattern = patterns_tag[id];
        if (pattern.currentFreq == 0) {
            feature.push_back(0);
            feature.push_back(0);
        ...
        for (int i = 0; i + 1 < pattern.size(); ++ i) {
            ...
            if (!pattern2id_tag.count(right.hashValue)) {
                cerr << i << " " << pattern.size() << endl;
                left.show();
                right.show();
            }
            assert(pattern2id_tag.count(left.hashValue));
            assert(pattern2id_tag.count(right.hashValue));

            PATTERN_ID_TYPE leftID = pattern2id_tag[left.hashValue], rightID = pattern2id_tag[right.hashValue];

            double current = patterns_tag[leftID].probability * patterns_tag[rightID].probability;
            if (current > best) {
                best = current;
                AB = leftID;
                CD = rightID;
        ...
        //cerr<<"F1: Qi look at here"<<endl;
        // prob_feature
        double f1 = pattern.probability / patterns_tag[AB].probability / patterns_tag[CD].probability;
        //cerr<<"F2: Qi look at here"<<endl;
        // occur_feature
        double f2 = patterns_tag[id].currentFreq / sqrt(patterns_tag[AB].currentFreq + EPS) / sqrt(patterns_tag[CD].currentFreq + EPS);
        //cerr<<"F3: Qi look at here"<<endl;
        // log_occur_feature
        double f3 = sqrt(patterns_tag[id].currentFreq) * log(f1);
        //cerr<<"F4: Qi look at here"<<endl;
        // prob_log_occur
        double f4 = patterns_tag[id].currentFreq * log(f1);
        ...
        vector<double> outside(pattern.size(), 0);
        double total = 0.0;
        for (TOTAL_TOKENS_TYPE ed : id2ends_tag[id]) {
            TOTAL_TOKENS_TYPE st = ed - patterns_tag[id].size() + 1;
    ...
    int recognize_tag(vector<Pattern> &truth) {
        fprintf(stderr, "Loaded Truth = %d\n", truth.size());
        int truthCnt = 0;
        for (int i = 0; i < truth.size(); ++ i) {
            if (pattern2id_tag.count(truth[i].hashValue)) {
                ++ truthCnt;
                PATTERN_ID_TYPE id = pattern2id_tag[truth[i].hashValue];
                patterns_tag[id].label = truth[i].label;
            }
        }
        fprintf(stderr, "Recognized PosTag Truth = %d\n", truthCnt);
        return truthCnt;
    }
    ...
    vector<vector<double>> extract_tag(vector<string> &featureNames) {
        // prepare token counts
        const TOTAL_TOKENS_TYPE& corpusTokensN = Documents::totalWordTokens;
        for (PATTERN_ID_TYPE i = 0; i < patterns_tag.size(); ++ i) {
            patterns_tag[i].probability = patterns_tag[i].currentFreq / (corpusTokensN / (double)patterns_tag[i].size());
        }
        ...
        for (PATTERN_ID_TYPE i = 0; i < patterns_tag.size(); ++ i) {
            if (patterns_tag[i].size() > 1) {
                ...
                //extractPunctuation(i, features[i]);
                //extractStopwords(patterns_tag[i], features[i]);
                features[i].shrink_to_fit();
            }
        }
    ...
    void augment_tag(vector<vector<double>> &features, Segmentation &segmentation, vector<string> &featureNames) {
        features = extract_tag(featureNames);
        return;
    }
};
